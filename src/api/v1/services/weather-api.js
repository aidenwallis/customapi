...
function getDirection(bearing) {
    for (const dir in cardinalDirections) {
...
function parseWeather(payload) {
    ...
    text += `The wind is blowing from the ${getDirection(data.wind.deg)} at `;
    text += `${convertToMPH(data.wind.speed)} (${convertToKMH(data.wind.speed)})`;
